...
void CConvertImageDlg::OnOk() 
{
	...
	if(dlg.DoModal() == IDOK)
	{
		m_FileName = dlg.GetPathName();		// 获取文件名及路径
...
void CConvertImageDlg::OnConvert() 
{
	if(TestButtonState(IDC_JPEG))	// 判断JPEG单选按钮是否被选中
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			...
			CLSID clsid;
			GetCodesClsid(L"image/jpeg",&clsid);		// 获取JPEG图像的CLSID
			bmp.Save(SaveName.AllocSysString(),&clsid);	// 保存为JPEG图像格式
		}
	}
	else if(TestButtonState(IDC_BMP))
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			...
			Bitmap bmp(m_FileName.AllocSysString());	// 定义一个位图对象
			CLSID clsid;
			GetCodesClsid(L"image/bmp",&clsid);		// 获取JPEG图像的CLSID
			bmp.Save(SaveName.AllocSysString(),&clsid);	// 保存为JPEG图像格式
		}
	}
	else
	{
		...
		if(dlg.DoModal() == IDOK)
		{
			...
			Bitmap bmp(m_FileName.AllocSysString());	// 定义一个位图对象
			CLSID clsid;
			GetCodesClsid(L"image/gif",&clsid);		// 获取JPEG图像的CLSID
			bmp.Save(SaveName.AllocSysString(),&clsid);	// 保存为JPEG图像格式
		}
	}
}
