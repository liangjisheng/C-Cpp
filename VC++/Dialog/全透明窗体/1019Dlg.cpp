...
BOOL CMy1019Dlg::OnInitDialog()
{
	...
	m_Edit.SetWindowText("将桌面背景设置为窗口背景\r\n");
	...
	if(SetLayeredWindowAttributes)
		SetLayeredWindowAttributes(GetSafeHwnd(),RGB(0,0,0),255,2);	// 255 用来调节透明度
	FreeLibrary(hInst);
...
void CMy1019Dlg::OnPaint() 
{
	...
	CPaintDC dc(this);
	PaintDesktop(dc.m_hDC);	// 将桌面背景绘制窗体上
}
...
void CMy1019Dlg::OnMove(int x, int y) 
{
	...
	CDC * pDC=GetDC();
	PaintDesktop(pDC->m_hDC);	// 窗体移动时重新将背景绘制到窗体上
}
