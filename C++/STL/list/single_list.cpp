...
pNode reverse_list_1(pNode head);
pNode reverse_list_recursion(pNode p,Node*& head);
...
int main()
{
	...
	// reverse_list_1
	// head=reverse_list_1(head);
	// print_list(head);

	// reverse_list_recursion
	// pNode p;
	// head=reverse_list_recursion(p,head);
...
pNode sort_list(pNode head) {
	...
	for(i=1;i<=len;i++) {
		for(j=1;j<=len-i;j++) {
			if(pre->data > p->data)
				swap_list_by_data(head,pre->data,p->data);
			pre=pre->next;
			p=p->next;
		}
		pre=head;
...
pNode reverse_list_recursion(pNode p,Node*& head)
{
	if(p==NULL || p->next==NULL) {
		...
		pNode tmp=reverse_list_recursion(p->next,head);
		tmp->next=p;
		return p;
	}
}