...
//	int count;//count表示此刻在一楼的人数，就是最早到达和最晚到达，两人到达时间
	...
	for(;i<n;)//n表示总共的人数，n为全局变量，i表示此刻正在坐电梯的人
	{
		//count=1;
	...
	//	if(count==1){
	//		for(j=i;j<n;j++){
	//			internal=ArriveTime[j]-ArriveTime[i-1];
	//		    wt=EleRunTime-internal;//第j个人的等待时间
	//			if(wt>=0){
	//				CurEleArrFloor.push_back(ArriveFloor[j+1]);
	//			    WaitTime.push_back(wt);
	//			    count++;//此刻在一楼等电梯的人数
	//			}
	//		}
	//		EleLoadNum.push_back(count);
	//		i+=count;//i也就是人数，往后移
	//	    MaxFloor=maxf(CurRunArrFloor);//求最大楼层数
	//	    StopTimeNum=DifferentFloorNum(CurRunArrFloor);//求去往不同楼层的人数
	//		EleRunTime=2*AFloorRunTime*MaxFloor+StopTime*StopTimeNum;
	//	}
	//}
